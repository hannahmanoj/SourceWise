    ...
    <main className="aura-bg min-h-screen px-6 py-8 text-[#171717] sm:px-10">
      <div className="mx-auto max-w-5xl">
        <header className="mb-10 flex flex-col justify-between gap-6 border-b border-black/10 pb-8 md:flex-row md:items-end">
          <div>
            <Link
              className="mb-8 inline-flex h-10 items-center rounded-full border border-black/10 bg-white px-4 text-sm font-semibold transition hover:bg-[#fbfaf7]"
              href="/research"
            >
              Back to research
            </Link>
          ...
          <div className="rounded-full border border-black/10 bg-white/70 px-5 py-3 text-sm font-semibold text-black/60">
            {mockResearch.papers.length} papers
          </div>
        </header>

        <PaperList papers={mockResearch.papers} showCompare={false} />
      </div>
    </main>
  );
}
